...
export default function AiTeacher() {
  ...
  const sendMessage = async () => {
    ...
    if (text.length > 2000) { toast.error('Text too long (max 2000 characters)'); return; }
    ...
    try {
      ...
      if (!resp.ok || !resp.body) {
        if (resp.status === 429) { toast.error('Rate limit reached.'); setMessages(prev => [...prev, { role: 'assistant', content: 'Rate limit reached.' }]); setIsLoading(false); return; }
        if (resp.status === 402) { toast.error('Usage limit reached.'); setMessages(prev => [...prev, { role: 'assistant', content: 'Usage limit reached.' }]); setIsLoading(false); return; }
      ...
      if (user && assistantSoFar) {
        ...
        if (mode === 'correct') {
          await supabase.from('grammar_corrections').insert({
            user_id: user.id, language_id: activeLanguage?.id || null,
            original_text: text, corrected_text: assistantSoFar.substring(0, 500), grammar_rule: mode,
          } as any);
        }
        await supabase.from('analytics_events').insert({
          user_id: user.id, event_type: 'ai_teacher', event_data: { mode, message_length: text.length } as any,
        });
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              {activeLanguage ? `${activeLanguage.flag_emoji} ${activeLanguage.name} · ` : ''}Grammar correction & practice
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          {modes.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => { setMode(id); setMessages([]); }}
              className={`flex-1 p-2 rounded-xl border-2 text-center transition-all ${mode === id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-3">
        ...
        {messages.map((msg, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm ${msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-tr-md' : 'bg-card shadow-card border border-border rounded-tl-md'}`}>
              {msg.role === 'assistant' ? (
        ...
        {isLoading && messages[messages.length - 1]?.role !== 'assistant' && (
          <div className="flex gap-2">
            <div className="w-7 h-7 rounded-lg bg-gradient-accent flex items-center justify-center flex-shrink-0"><GraduationCap className="w-3.5 h-3.5 text-accent-foreground" /></div>
      ...
      <div className="px-4 py-3 border-t border-border bg-card/80 backdrop-blur">
        <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-2">
          <Textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder={placeholders[mode]}
            className="flex-1 min-h-[40px] max-h-[100px] resize-none" disabled={isLoading} rows={1}
