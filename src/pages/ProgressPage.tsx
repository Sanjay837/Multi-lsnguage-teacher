...
export default function ProgressPage() {
  ...
  const { data: correctionCount } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await supabase.from('grammar_corrections' as any).select('*', { count: 'exact', head: true });
      return count || 0;
    },
  });

  const { data: pronunciationHistory } = useQuery({
    queryKey: ['pronunciation-trends', langId],
    queryFn: async () => {
      let query = supabase.from('pronunciation_practice').select('accuracy_score, created_at')
  ...
  const avgScore = totalCompleted > 0
    ? Math.round(progress!.filter(p => p.completed).reduce((sum, p) => sum + (p.max_score > 0 ? (p.score / p.max_score) * 100 : 0), 0) / totalCompleted) : 0;
  ...
  const suggestedLessons = lessons?.filter(l => !progress?.find(pr => pr.lesson_id === l.id)?.completed).slice(0, 3) || [];
  ...
  const weakCategories = Object.entries(categoryScores)
    ...
    .filter(c => c.avg < 80).sort((a, b) => a.avg - b.avg);
    ...
    <div className="px-4 pt-8 space-y-6 pb-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {activeLanguage ? `${activeLanguage.flag_emoji} ${activeLanguage.name} · ` : ''}Track your learning journey
        </p>
      </div>
      ...
      {topIncorrect.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="p-4 shadow-card">
            <div className="flex items-center gap-2 mb-3"><AlertTriangle className="w-4 h-4 text-warning" /><h3 className="text-sm font-semibold">Words to Review</h3></div>
            <div className="space-y-2">
              {topIncorrect.map(([word, count]) => (
                <div key={word} className="flex items-center justify-between"><span className="text-sm">{word}</span><span className="text-xs text-muted-foreground">missed {count}x</span></div>
              ))}
            </div>
            <Button variant="outline" size="sm" className="w-full mt-3" onClick={() => navigate('/ai-teacher')}>Practice weak words with AI Teacher</Button>
          </Card>
        </motion.div>
      )}
      ...
      {weakCategories.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
          <Card className="p-4 shadow-card">
            <div className="flex items-center gap-2 mb-3"><Lightbulb className="w-4 h-4 text-accent" /><h3 className="text-sm font-semibold">Areas to Improve</h3></div>
            <div className="space-y-3">
              {weakCategories.map(({ category, avg }) => (
                <div key={category}><div className="flex items-center justify-between mb-1"><span className="text-sm capitalize">{category}</span><span className="text-xs text-muted-foreground">{avg}%</span></div><Progress value={avg} className="h-1.5" /></div>
              ))}
            </div>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <div className="space-y-2">
            {suggestedLessons.map(lesson => (
              <Card key={lesson.id} className="p-3 shadow-card cursor-pointer hover:shadow-elevated transition-shadow" onClick={() => navigate(`/lessons/${lesson.id}`)}>
                <div className="flex items-center justify-between"><div><p className="text-sm font-medium">{lesson.title}</p><p className="text-[10px] text-muted-foreground capitalize">{lesson.category} · Level {lesson.difficulty_level}</p></div><ArrowRight className="w-4 h-4 text-primary" /></div>
              </Card>
            ))}
          </div>
        </motion.div>
      ...
      {pronunciationHistory && pronunciationHistory.length > 2 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.32 }}>
          <Card className="p-4 shadow-card">
            <div className="flex items-center gap-2 mb-3"><Mic className="w-4 h-4 text-primary" /><h3 className="text-sm font-semibold">Pronunciation Accuracy Trend</h3></div>
            <div className="h-40">
            ...
            {progress.slice(-5).reverse().map(p => (
              <Card key={p.id} className="p-3 shadow-card">
                <div className="flex items-center justify-between">
                  <div><p className="text-sm font-medium">{p.completed ? '✅' : '📝'} Lesson</p><p className="text-xs text-muted-foreground">Score: {p.score}/{p.max_score} · {formatTime(p.time_spent_seconds)}</p></div>
