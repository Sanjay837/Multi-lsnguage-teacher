...
const scenarios = [
  { id: 'interview', label: 'Job Interview', icon: Briefcase, desc: 'Practice interview Q&A', prompt: 'Simulate a job interview in Kannada. Ask typical interview questions one by one, correct mistakes, and provide both Kannada script and English translation.' },
  { id: 'travel', label: 'Travel & Directions', icon: Plane, desc: 'Navigate new places', prompt: 'Simulate asking for directions and travel conversations in Kannada. Role-play as a local helping a traveler. Include Kannada script and English translation.' },
  { id: 'daily', label: 'Daily Conversation', icon: Coffee, desc: 'Everyday small talk', prompt: 'Simulate everyday casual conversations in Kannada - greetings, weather, family talk. Be natural and conversational. Include Kannada script and English translation.' },
  { id: 'shopping', label: 'Shopping', icon: ShoppingCart, desc: 'Market & store dialogues', prompt: 'Simulate shopping conversations in Kannada - bargaining at a market, buying groceries, asking prices. Include Kannada script and English translation.' },
  { id: 'phone', label: 'Phone Call', icon: Phone, desc: 'Telephone etiquette', prompt: 'Simulate a phone conversation in Kannada - making appointments, customer service calls, calling a friend. Include Kannada script and English translation.' },
];
...
export default function Scenarios() {
  ...
  const startScenario = (scenario: typeof scenarios[0]) => {
    ...
    sendToAI(scenario, [{ role: 'user', content: `Start the ${scenario.label} scenario. Begin the role-play with the first line in Kannada.` }]);
  };
  ...
  const sendToAI = async (scenario: typeof scenarios[0], msgs: ChatMessage[]) => {
    ...
    try {
      ...
      const resp = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({ messages: [systemMsg, ...msgs.slice(-12)] }),
      });
    ...
    return (
      <div className="px-4 pt-8 space-y-6 pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Practice conversations in realistic situations</p>
        </div>
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      <div className="px-4 pt-6 pb-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">{selectedScenario.desc}</p>
        </div>
      </div>
        ...
        {messages.map((msg, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm ${
              msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-tr-md' : 'bg-card shadow-card border border-border rounded-tl-md'
            }`}>
              {msg.role === 'assistant' ? (
