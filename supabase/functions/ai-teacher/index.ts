...
serve(async (req) => {
  ...
  try {
    const { text, mode, targetLanguage, history } = await req.json();
...
3. Explain each correction in Kannada (ಕನ್ನಡ) AND English
...
**Corrections:**
- [Each correction with explanation in both Kannada and English]
...
2. 2 translation exercises (English → Kannada)
3. 1 sentence construction exercise

Include answers at the bottom. Explain in both Kannada and English.
...
2. Provide the explanation in Kannada (ಕನ್ನಡ) as well
